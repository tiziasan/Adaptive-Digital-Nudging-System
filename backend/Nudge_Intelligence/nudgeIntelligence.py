...
class NudgeIntelligence:
    ...
    Selects optimal nudging strategy and generates personalized nudges using LLM
    """
    ...
    def __init__(self, api_key: str = None, use_ollama: bool = False, model: str = "qwen3:14b"):
        ...
        # 23 nudging strategies from literature with detailed descriptions
    ...
    def strategy_optimizer(self, complete_data: Dict[str, Any], fairness_prompt:str,explainability_prompt:str) -> Dict[str, str]:
        """
        Component 1: Strategy Optimizer
        Selects the most appropriate nudging strategy based on user profile

        Args:
            complete_data: User modeling data from Layer 2 (just the user_modeling section)

        Returns:
            Dictionary with selected strategy and reasoning for the selection
        """
        # Prepare the list of strategies with descriptions for the prompt
...
"""
    ...
    def nudge_generator(self,
        ...
        Creates a personalized nudge message for sustainability improvement

        Args:
            selected_strategy: The nudging strategy selected by strategy_optimizer
            user_modeling_data: Complete JSON data including user_modeling, electrodomestic_interactions,
                          context, and previous_nudges

        Returns:
            String: Personalized nudge message (max 3 lines)
        """
...
"""
    ...
    def create_nudge(self, complete_data: Dict[str, Any], fairness_prompt:str,explainability_prompt:str, ethics_prompt:str) -> Dict[str, Any]:
        """
        Runs both components: selects strategy and generates nudge

        Args:
            complete_data: Complete user profile from Layer 2 including all data
                          (user_modeling, electrodomestic_interactions, context, previous_nudges)

        Returns:
            Dictionary with strategy, reasoning, nudge message, and merged data
        """
        # Extract just the user_modeling section for strategy selection
        user_modeling_data = {k: v for k, v in complete_data.items() if k != 'previous_nudges'}

        #user_modeling_data = complete_data.get("user_modeling", {})

        # Step 1: Select optimal strategy (now returns dict with strategy + reasoning)
        ...
        # Step 2: Generate personalized nudge using complete data
        nudge_message = self.nudge_generator(selected_strategy, user_modeling_data, fairness_prompt, ethics_prompt)

        # Create nudge intelligence results with explainability
        nudge_results = {
